...
import numpy as np
# Import plotting library (optional, but good for visualizing ROC)
...
# Ensure target variable y is suitable for ROC (e.g., 0 and 1 or -1 and 1)
# If y is -1 and 1, we might need to adjust for roc_curve which often expects 0 and 1
# Let's map -1 to 0 for ROC calculation purposes if needed, but keep original y for training
...
if max_pca_components <= 0:
    ...
    X_processed = X # Use original (unscaled) features for RF if skipping PCA
else:
    ...
    try:
        ...
        X_processed = X_reduced # Use PCA-reduced features for the model
        pca_applied = True
        ...
        print("Using original features instead of PCA.")
        X_processed = X
        pca_reducer = None
...
print("\nStarting Leave-One-Out Cross-Validation (using " + ("PCA features" if pca_applied else "original features") + ")...")
# Perform LOOCV
